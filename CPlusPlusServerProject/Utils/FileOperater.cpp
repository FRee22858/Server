...
bool FileOperater::OpenFile(const char* name)
{
	if (IfValid())
	{
		Close();
...
bool FileOperater::SeekFile(int pos, enumFilePOSType from)
{
	if (!IfValid()) return 0;
	int seek_from = SEEK_SET;
	if (from == endPos) seek_from = SEEK_END;
	else if (from == currentPos) seek_from = SEEK_CUR;
...
int FileOperater::GetFirstLine(char*&line_buf, int& max_len)
{
	SeekFile(0);
	return GetNextLine(line_buf, max_len);
}
...
int FileOperater::GetNextLine(char*&line_buf, int& max_len)
{
	if (!IfValid()) return -1;

	int c, i = 0;//i来记录字符串长度，c来存储字符
	//buf为null或max_len为0时动态为其分配空间
	if (line_buf == NULL || max_len == 0)
	{
		line_buf = (char*)malloc(10);
		max_len = 8;
	}
	while ((c = fgetc(mFile)) != '\n')
	{
		...
		{
			max_len = max_len + 8;
			line_buf = (char*)realloc(line_buf, max_len);//空间不足时，重新进行分配
			*(line_buf + i++) = c;
		}
	}
	*(line_buf + i++) = '\n';
...
const char* FileOperater::GetFilePosfix(const char* path)
{
